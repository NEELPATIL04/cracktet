...
export default function PublicResourcesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Resources (attempt to get PDFs using same API you use in ResourcesPage)
      const resourcesResponse = await fetch("/api/resources");
      if (resourcesResponse.ok) {
        const resourcesData = await resourcesResponse.json();
        setResources(resourcesData.resources || []);
        setResourcesAuthRequired(false);
      } else if (resourcesResponse.status === 401) {
        // API requires authentication to fetch full PDFs
        setResources([]);
        setResourcesAuthRequired(true);
      } else {
        // other non-ok statuses — just show nothing (or handle as needed)
        setResources([]);
        setResourcesAuthRequired(false);
      }
    } catch (error) {
  ...
  const handleResourceClick = (resourceUuid: string) => {
    console.log("Handle Resource Client function called");
    router.push(`/resources/view/${resourceUuid}`);
  };
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-600">
                Access comprehensive study materials and videos to boost your
                TET preparation
              </p>
            </div>
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-l-4 border-yellow-400 p-4 rounded-lg mb-6">
            ...
            <p className="text-yellow-700 text-sm">
              You can preview videos below. Login to access PDF study materials
              and get full access to all educational content!
            </p>
          </div>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-sm text-gray-600">
              {resources.length} PDF{resources.length !== 1 ? "s" : ""} found
            </p>
          </div>

          {resourcesAuthRequired ? (
            <div className="bg-white rounded-lg p-6 border border-dashed border-gray-200 text-center">
              <p className="text-gray-700 mb-4">
                PDF access requires an account. Login to view and open full
                PDFs.
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {resources.map((res) => (
                <div
                  key={`resource-${res.id}`}
                  className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 overflow-hidden border group cursor-pointer"
                >
                  <div className="p-4">
                    ...
                    <button
                      onClick={() => handleResourceClick(res.uuid)}
                      className="flex items-center justify-center space-x-2 w-full bg-primary hover:bg-primary-dark text-white py-2 px-3 rounded-lg transition-colors font-medium text-sm group-hover:shadow-lg"
                    >
